...
namespace Bai2
{
    public partial class frmBai2 : Form
    {
        ...
        {
            TaoBangHoanVi(); string s = "";
            for (int i = 0; i < 26; i++) s += BangHoanVi[i];
            MessageBox.Show(s, "Bảng hoán vị ngẫu nhiên");
        }
        char[] BangHoanVi = new char[26];
        private void TaoBangHoanVi()
        {
            for (int i = 0; i < 26; i++) BangHoanVi[i] = (char)(65 + i);
            Random RndObj = new Random(DateTime.Now.Millisecond);
            int SoLan = 20 + RndObj.Next(80); for (int n = 0; n < SoLan; n++)
            {
                int n1 = RndObj.Next(26); int n2 = RndObj.Next(26);
                char c = BangHoanVi[n1]; BangHoanVi[n1] = BangHoanVi[n2]; BangHoanVi[n2] = c;
            }
        }
