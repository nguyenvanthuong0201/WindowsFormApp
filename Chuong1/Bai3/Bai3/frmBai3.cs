    ...
    public partial class Form1 : Form
    {
        ...
        private bool KiemTraMaKhoaHopLe()
        {
            if (rbSo.Checked) {
                ...
                {
                    char c = txtMaKhoa.Text[i]; if (c < '1' || c > '8') return false;
                    else
                        CacSo[(int)c - 49] = true;
                }
                for (int i = 0; i < 8; i++)
            ...
            {
                bool[] CacChu = new bool[26]; if (txtMaKhoa.Text.Length < 8) return false;
                List<char> dsChu = new List<char>(); for (int i = 0; i < txtMaKhoa.Text.Length; i++)
                {
                    char c = txtMaKhoa.Text[i]; if (c < 'A' || c > 'Z') return false;
                    else {
                        if (dsChu.Contains(c)) return false; dsChu.Add(c);
                    }
                }
                dsChu.Sort(); string ss = "";
                for (int i = 0; i < txtMaKhoa.Text.Length; i++)
                { KhoaSo[i] = dsChu.IndexOf(txtMaKhoa.Text[i]);
                    ss += KhoaSo[i].ToString() + " ";
                }
                MessageBox.Show(ss); return true;
            }
        }
            ...
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 8; j++)
                {
                    ...
                    if (ii < txtBanTinRo.Text.Length)
                        ...
                        MaTran[i, j] = 'Z'; }// Ghép kết quả    
            string s = "";if (rbDongVaCot.Checked)
            {
                // Theo dòng và cột  
                for (int i = 0; i < 8; i++)
                    for (int j = 0; j < 8; j++)
                ...
                (int ii=0; ii<15; ii++)    
                {
                    if (ii <= 7)
                    {
                        ...
                        {
                            for (int j = ii;
                                j >= 0; j--)s += MaTran[ii - j, KhoaSoNguoc[j]];
                            // Chéo xuống     
                        }
                        else
                        {
                            for (int j = 0; j <= ii; j++)
                                s += MaTran[ii - j, KhoaSoNguoc[j]];
                            // Chéo lên          
                        }
                    }
                    else
        ...
        {
            TaoKhoaSoNguoc(); if (txtBanTinDuocMaHoa.Text.Length != 64) { MessageBox.Show("Không thể giải mã được"); return; }
            if (rbDongVaCot.Checked)
            {
                ...
                // Theo đường chéo 
                int ij = 0;for (int ii = 0; ii < 15; ii++)
                { if (ii <= 7)
                    { if (ii % 2 == 0)
                        { for (int j = ii; j >= 0; j--)
                                MaTran[ii -j, KhoaSoNguoc[j]] = txtBanTinDuocMaHoa.Text[ij++];
                            // Chéo xuống 
                        }
                        else
                        {
            ...
            string s = "";
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 8; j++)s += MaTran[i, j];
